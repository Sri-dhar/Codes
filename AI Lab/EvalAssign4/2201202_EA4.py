...
class GameStones:
    ...
    def create_game_tree(self):
        with open("game_tree.txt", "w") as file:
            self._create_game_tree(self.num_stones, file)

    def _create_game_tree(self, num_stones, file):
        if num_stones == 0:
            file.write("0\n")
            return
        file.write(f"{num_stones} -> ")
        if num_stones >= 2:
            file.write(f"{num_stones - 1}, {num_stones - 2}\n")
            self._create_game_tree(num_stones - 1, file)
            self._create_game_tree(num_stones - 2, file)
        else:
            file.write(f"{num_stones - 1}\n")
            self._create_game_tree(num_stones - 1, file)
